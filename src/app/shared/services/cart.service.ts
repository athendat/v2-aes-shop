...
import { Cart, CartItem, Product, RestResponse } from "../types";
...
export class CartService {
    ...
    getCartItems(): Observable<CartModel> {
        ...
        return this.#http.get<any>(`${environment.API_URL}/cart/shop`)
            .pipe(
                map(({ data }) => {
    ...
    createCart(cart: Cart): Observable<RestResponse<Cart>> {
        return this.#http.post<RestResponse<Cart>>(`${environment.API_URL}/cart`, cart);
    }
    ...
    findCart(): Observable<RestResponse<Cart>> {
        return this.#http.get<RestResponse<Cart>>(`${environment.API_URL}/cart`).pipe(
            tap(response => {
    ...
    saveCart(cart: Cart): Observable<RestResponse<Cart>> {
        return this.#http.patch<RestResponse<Cart>>(`${environment.API_URL}/cart`, cart);
    }
}
